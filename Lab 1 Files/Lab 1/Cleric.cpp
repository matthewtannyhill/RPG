...
Cleric::Cleric(string name, int maxhp, int currenthp, int strength, int speed, int magic):
...
bool Cleric::useAbility() //FINISHED
{
    if (mana >= CLERIC_ABILITY_COST) //if has enought mana
    {
        if ((magic / 3) < 1) //if the amount changed will be less than one, add one HP
        {
            HP = HP + 1;
        }
        else // if not, add normal
        {
            HP = (HP + (magic / 3));
        }

        mana = (mana - CLERIC_ABILITY_COST); // deplete mana

        if (HP > max_HP) //if it will go above max HP, HP equals max HP
        {
                HP = max_HP;
        }
    ...
    else //if it doesn't have enough mana, it can't be used.
    {
        return false;
    }
}
...
void Cleric::regenerate() //FINISHED
{
    Fighter::regenerate();
